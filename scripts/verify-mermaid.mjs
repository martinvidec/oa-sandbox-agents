...
// Exit-Code: 0 = keine Befunde, 1 = Befunde, 2 = Setup-/Aufruffehler.
...
let outDir = path.join(os.tmpdir(), 'verify-mermaid');
...
const diagrams = await page.evaluate(() => {
  ...
  document.querySelectorAll('.mermaid').forEach((m, idx) => {
    ...
    r.errorBoxes = svg.querySelectorAll('.error-icon, .error-text').length + (svg.getAttribute('aria-roledescription') === 'error' ? 1 : 0);
...
// Screenshots: bevorzugt der umgebende .diagram-container, sonst der .mermaid-Block selbst
const screenshots = [];
let targets = await page.$$('.diagram-container');
if (targets.length === 0) targets = await page.$$('.mermaid');

...
3. Befunde gehen als PR-Kommentare raus; der Reviewer editiert **keinen** Code. Ohne Befunde: ein Kommentar „Review ok, keine Befunde" — Schweigen zählt nicht als Freigabe.
   - **Diagramm-PRs brauchen einen Screenshot-Beleg.** Ändert ein PR ein Diagramm (z.B. Mermaid in `docs/workflow-hochglanz.html`), lässt der Coder lokal `scripts/verify-mermaid.mjs` laufen (Setup: [`verify-setup.md`](verify-setup.md)) und legt vor `agent:review` einen PR-Kommentar mit den Screenshots und dem Ergebnis (`ok`/`findings` aus dem Report) an — ein neuer Kommentar, wie jeder Beleg (Häkchen nur mit Beleg, unten). Fehlt der Kommentar oder meldet der Report Befunde (Error-Boxen, Überlappungen, abgeschnittene Labels), ist das ein Review-Befund; visuelle Akzeptanzkriterien gelten ohne diesen Beleg nicht als erfüllt.
